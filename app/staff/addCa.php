...
if ($_SERVER["REQUEST_METHOD"]=="POST")
   {
      ...
      //check for empty variable
        if(empty($stud_regnumber) || empty($ca_scores) || empty($studentClass) || empty($studentClass_arm) || empty($subject))
        {
            exit("Please fill all the fields...");
        }else{
            //CALL METHOD TO ADD CA
            $staff_module->addCa($ca_scores,$stud_regnumber,$subject,$studentClass_arm,$staffid,$schid,$dateCreated);
        }
   }
else
   {
